...
public :
   ...
   reduceTree(std::string filename);
...
#ifdef reduceTree_cxx
reduceTree::reduceTree(std::string filename)
{
  inputFile_ = filename;
  TTree* tree;
...
// used to generate this class and read the Tree.
  ...
  if (!f) {
    ...
    std::string temp = filename + ":/skimLaurent";
    f->cd(temp.data());
  }
...
Int_t reduceTree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t reduceTree::LoadTree(Long64_t entry)
{
...
void reduceTree::Init(TTree *tree)
{
...
void reduceTree::Show(Long64_t entry)
{
...
Int_t reduceTree::Cut(Long64_t entry)
{
