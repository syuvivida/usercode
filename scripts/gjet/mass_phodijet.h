...
// Tue Jul 26 11:59:35 2011 by ROOT version 5.30/00
// from TTree EventTree/Event data
// found on file: job_photon_2011a_PR_v4_Jul6_JSON_noskim.root
//////////////////////////////////////////////////////////

#ifndef mass_phodijet_h
#define mass_phodijet_h
...
/*    mass_phodijet(TTree *tree=0); */
   mass_phodijet(std::string inputFile);
   virtual ~mass_phodijet();
   ...
   virtual Bool_t   isGoodJet(Long64_t entry, Int_t ijet);

};

#endif

#ifdef mass_phodijet_cxx
//mass_phodijet::mass_phodijet(TTree *tree)
mass_phodijet::mass_phodijet(std::string inputFile)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
  cout << "Reading in " << inputFile.data() << endl;
  TTree *tree;
...
mass_phodijet::~mass_phodijet()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t mass_phodijet::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t mass_phodijet::LoadTree(Long64_t entry)
{
...
void mass_phodijet::Init(TTree *tree)
{
...
void mass_phodijet::Show(Long64_t entry)
{
...
Int_t mass_phodijet::Cut(Long64_t entry)
{
