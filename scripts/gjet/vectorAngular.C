#define vectorAngular_cxx
#include "vectorAngular.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	// check if this is a real photon
  	if(genPhoPt->at(imc)< 20.0)continue;
    	if(fabs(genPhoEta->at(imc)) > 2.5)continue;


	if(genPhoMomPID->at(imc)==22 && !findADirPhoton)
	  {
	...
	if(fabs(genPhoMomPID->at(imc)) < 22 && !findAFraPhoton)
	  {
	...
	if(genJetPt->at(ijet) > genJetMaxPt)
	  {
	    ...
	    genJetMaxPt = genJetPt->at(ijet);
	    leadingJetIndex= ijet;
	  }
	else if(genJetPt->at(ijet) > secondJetMaxPt)
	  {
	    secondJetMaxPt = genJetPt->at(ijet);
 	 ...
 	 &&
 	 (
  	  genJetPt->at(leadingJetIndex) > 20.0 &&
 	  fabs(genJetEta->at(leadingJetIndex)) < 3.0
 	  )
	 )
	findLeadingJet = true;
	 ...
	 &&
 	 (
  	  genJetPt->at(secondLeadingJetIndex) > 20.0 &&
 	  fabs(genJetEta->at(secondLeadingJetIndex)) < 3.0
 	  )
	 )
      ...
      if(findLeadingJet)
	{
	  gen_1stjet.SetPtEtaPhiE(
				  genJetPt->at(leadingJetIndex),
				  genJetEta->at(leadingJetIndex),
				  genJetPhi->at(leadingJetIndex),
				  genJetE->at(leadingJetIndex)
				  );
      ...
      if(findSecondLeadingJet)
	{
	  gen_2ndjet.SetPtEtaPhiE(
				  genJetPt->at(secondLeadingJetIndex),
				  genJetEta->at(secondLeadingJetIndex),
				  genJetPhi->at(secondLeadingJetIndex),
				  genJetE->at(secondLeadingJetIndex)				  
				  );
