{
  ...
  std::string cut   = "nVtx>0 && PHOLEAD_isEBEEGap==0 && PHOLEAD_isEBGap==0 && PHOLEAD_isEEGap==0 && PHOLEAD_isTransGap==0 && abs(PHOLEAD_eta)<1.44";
  gROOT->ProcessLine(".L compareTwoTrees.C");

  int decCode = 1;

  compareTwoTrees(dataF,mcF,treeName,"PHOLEAD_energy",decCode,"energy",25,0.,25.,"E(#gamma) [GeV]","",cut);
  ...
  compareTwoTrees(dataF,mcF,treeName,"PHOLEAD_covPhiPhi",decCode,"covphiphi",20,0,0.0005,"covPhiPhi","",cut);

  compareTwoTrees(dataF,mcF,treeName,"PHOLEAD_covEtaPhi",decCode,"covetaphi",20, -0.0003,0.0003,"covEtaPhi","",cut);

  compareTwoTrees(dataF,mcF,treeName,"PHOLEAD_covEtaEta",decCode,"covetaeta",20,0,0.0005,"covEtaEta","",cut);
