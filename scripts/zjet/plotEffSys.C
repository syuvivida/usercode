...
void plotEffSys(std::string filename)
{
  ...
  std::string ytitle = "Efficiency Systematics/Central";
  ...
  leg->SetBorderSize(0);
  leg->AddEntry(r_zy_up, "Up");
  leg->AddEntry(r_zy_down,"Down");
  leg->Draw("same");
  ...
  d->Draw("same");
  leg->Draw("same");
  c1->Print("eff/effsys.eps");
  c1->Print("eff/effsys.gif");
  c1->Print("eff/effsys.pdf");
