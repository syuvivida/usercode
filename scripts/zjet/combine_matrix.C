		    ...
		    bool logScale=false)
{
  ...
  for(int i=1; i<=nbin; i++){
    ...
    errorM(0,0) = total_e_2;
    errorM(1,1) = total_m_2;
    errorM(0,1) = errorM(1,0) = correlation*sys_e*sys_m;
    ...
    for(int in=0; in < NELE; in++)U(in)=1.0;

    TVectorD alpha(NELE);
    alpha = (errorInverse*U);
    alpha *= (1.0/(U*(errorInverse*U)));

    cout << "alpha(0) = " << alpha(0) << "\t alpha(1) = " << alpha(1) << endl;

    TVectorD measurement(NELE);
    measurement(0) = value_e;
    measurement(1) = value_m;
    double combined_value = measurement*alpha;
