...
void getKengJESSysRange(std::string var, bool update=false, double xmin=-9999.0, double xmax=-9999.0)
{
  TH1D* hcentral;
  TH1D* hup;
  TH1D* hdown;
  ...
  TFile *fmc  = TFile::Open("/afs/cern.ch/work/s/syu/angular_combine/"
  ...
  TFile *fraw = TFile::Open("/afs/cern.ch/work/s/syu/angular_combine/"
  ...
  TFile* outFile = new TFile(Form("kengJESSys.root"),
			     command.data());       
  r_down->Write();
  r_up->Write();
  r_corr->Write();
