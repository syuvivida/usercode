...
void getAnilEffSysRange(std::string var, bool update=false, double xmin=-9999.0, double xmax=-9999.0)
{
  ...
  hcentral    = (TH1D*)(fmc->Get(anilName.data()));
  ...
  for(int ib=1; ib<= r_corr->GetNbinsX(); ib++){
    ...
    double upDiff = hup->GetBinContent(ib)*scale_up - 
      hcentral->GetBinContent(ib)*scale_central;

    double dnDiff = hdown->GetBinContent(ib)*scale_down - 
      hcentral->GetBinContent(ib)*scale_central;

    double statErr = hcentral->GetBinError(ib);

    r_up  ->SetBinContent(ib, sqrt(upDiff*upDiff+ statErr*statErr));

    r_down->SetBinContent(ib, sqrt(dnDiff*dnDiff+ statErr*statErr));
