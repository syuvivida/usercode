...
Double_t ftotal_pos(Double_t *x, Double_t *par, Double_t& err) {  
  ...
  Double_t br = par[1]*background_pos->GetBinContent(bin_background);

  double sigerr = signal_pos->GetBinError(bin_signal);
  double bkgerr = background_pos->GetBinError(bin_background);
  
 
  err = sqrt(par[0]*par[0]*sigerr*sigerr +
	     par[1]*par[1]*bkgerr*bkgerr);
...
// -- main function
void fit_neve_chi2(TH1D* dataInput, TH1D* sigTemplate, TH1D* bkgTemplate, std::string prefix, Double_t& nsig, Double_t& nsig_err)
{
  ...
  if ( ierflg == 0 ) 
    {
      ...
      chi2 = gMinuit->fAmin;
      gMinuit->GetParameter(0, nsig, nsig_err); 
      gMinuit->GetParameter(1, nbkg, nbkg_err);  
      cout << "Ntotal =" << data->Integral(0,1000) << endl;
      ...
      char result[300];
      sprintf(result,"nsig = %.1lf #pm %.1lf",nsig,nsig_err);
