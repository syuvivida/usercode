...
void call_binL_MCtest(){
  ...
  for(int ieta = 0; ieta < nEtaBin; ieta++){
     for(int ipt=0; ipt < nPtBin; ipt++){
       ...
       ratioErr(nsigfit, errnsigfit, nbkgfit, errnbkgfit,
		purityfit, errpurityfit);
       
       if(ieta==0){
	 purity_twobin_EB[ipt]=purityfit;
	 purity_err_twobin_EB[ipt]=errpurityfit;
	 nsig_twobin_EB[ipt]= nsigfit;
	 nsig_err_twobin_EB[ipt]= errnsigfit;
       }
	 
       else if(ieta==1){
	 purity_twobin_EE[ipt]=purityfit;
	 purity_err_twobin_EE[ipt]=errpurityfit;
	 nsig_twobin_EE[ipt]= nsigfit;
