...
// 					    "",50,-5.0,5.0);
  ...
  for(int ieta=0; ieta< nEtaBin; ieta++){
	...
	htoyMC_data = (TH1F*)hdata_data[ieta][ipt]->Clone();

// 	for(int jbin=1; jbin<= htoyMC_sig->GetNbinsX(); jbin++){
	  for(int ibin=1; ibin<= htoyMC_sig->GetNbinsX(); ibin++)
	    {
	      float neve = //(jbin==ibin)?
		gRandom->Gaus(hTemplate_S[ieta][ipt]->GetBinContent(ibin),
			      hTemplate_S[ieta][ipt]->GetBinError(ibin))
		//:hTemplate_S[ieta][ipt]->GetBinContent(ibin)
		;		
	      htoyMC_sig->SetBinContent(ibin, neve);
// 	      cout << "nsig in bin " << ibin << " = " << neve << endl;

	      float neve2 = gRandom->Gaus(hTemplate_B[ieta][ipt]->GetBinContent(ibin),
					  hTemplate_B[ieta][ipt]->GetBinError(ibin));

	      htoyMC_bkg->SetBinContent(ibin, neve2);
// 	      cout << "nbkg in bin " << ibin << " = " << neve2 << endl;
// 	      cout << "original number " << hTemplate_B[ieta][ipt]->GetBinContent(ibin) << " +- " << 
	      // 	      hTemplate_B[ieta][ipt]->GetBinError(ibin) << endl;
	    }
...
// 	Double_t pull = (nsigtemplate - nsig_data[ieta][ipt])/errnsigtemplate;
	Double_t pull = (nsigtemplate - nsig_data[ieta][ipt])/nsig_data[ieta][ipt];
...
// 	  }
  ...
  for(int ieta=0; ieta < nEtaBin; ieta++){
    TFile* outFile = new TFile(Form("templateStatSys_%s.root",dec[ieta]),
			       "recreate");
