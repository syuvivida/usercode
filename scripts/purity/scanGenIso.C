...
void scanGenIso(std::string outputName="", std::string var=
  ...
  cout << "output file prefix = " << outputName << endl;
   ...
   for(int ieta = 0; ieta < nEtaBin; ieta++){
     ...
     for(int ipt=0; ipt < nPtBin; ipt++){
       ...
       TCut signalCut = genIsoCut + "isGenMatched && abs(genMomId) <= 22";
       TCut backgroundCut = !signalCut;

       hIsoPho_1[ieta][ipt] = (TH1F*)hTemplate->Clone();
       sprintf(tmp,"PhotonJet_Template_Et_%d_%d_Eta_%.2f_%.2f",(int)fBinsPt[ipt], (int)fBinsPt[ipt+1],
       ...
       sprintf(tmp,"AllMC_TemplateS_Et_%d_%d_Eta_%.2f_%.2f",(int)fBinsPt[ipt], (int)fBinsPt[ipt+1],
	       fBinsEta[ieta*2],fBinsEta[ieta*2+1]);
       ...
       sprintf(tmp,"AllMC_TemplateB_Et_%d_%d_Eta_%.2f_%.2f",(int)fBinsPt[ipt], (int)fBinsPt[ipt+1],
	       fBinsEta[ieta*2],fBinsEta[ieta*2+1]);
       ...
       sprintf(tmp,"%d < p_{T}(#gamma) < %d GeV, %.2f < |#eta(#gamma)| < %.2f",
	       (int)fBinsPt[ipt], (int)fBinsPt[ipt+1],
	       fBinsEta[ieta*2],fBinsEta[ieta*2+1]);
       hIsoPho_3[ieta][ipt]->SetTitle(tmp);
       hIsoPho_3[ieta][ipt]->SetXTitle("ecalIso+hcalIso+trackIso [GeV]");
       

       cout << "making signal histograms from photon + jet MC samples" << endl;
       TCut allCut   = basicCut + signalCut;
       makePlot(phoTree,phoWeight,phoPtHatLo,phoPtHatHi,Form("%s",var.data()),allCut,hIsoPho_1[ieta][ipt],normalize);

       cout << "making signal histograms from mixed MC samples" << endl;
       allCut = basicCut + signalCut;
       makePlot(mixTree,mixWeight,mixPtHatLo,mixPtHatHi,Form("%s",var.data()),allCut,hIsoPho_2[ieta][ipt],normalize);

       cout << "making background histograms from mixed MC samples" << endl;
       allCut = basicCut + backgroundCut;
   ...
   TLegend* leg = new TLegend(0.38,0.6,0.73,0.9);
   leg->SetFillColor(0);
   leg->SetFillStyle(0);
   ...
   for(int ieta = 0; ieta < nEtaBin; ieta++){
     for(int ipt=0; ipt < nPtBin; ipt++){
       ...
       hIsoPho_3[ieta][ipt]->GetYaxis()->SetDecimals();
       hIsoPho_3[ieta][ipt]->SetMarkerColor(kBlue);
       hIsoPho_3[ieta][ipt]->SetLineColor(kBlue);
       ...
       else
	 {
       ...
       leg->Clear();
       sprintf(tmp,"genIso < %d GeV for signal",genIsoCutValue);
       leg->SetHeader(tmp);
       leg->AddEntry(hIsoPho_1[ieta][ipt],"#gamma+jet MC signal");
       leg->AddEntry(hIsoPho_2[ieta][ipt],"#gamma+jet and dijet MC signal");
       leg->AddEntry(hIsoPho_3[ieta][ipt],"#gamma+jet and dijet MC background");
       leg->Draw("same");
