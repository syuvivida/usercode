...
{      
    ...
    for(int file=0; file<N_SAMPLES; file++) { 
	    ...
	    for(int type=0; type<SB_TYPE; type++) {
	      ...
	      sprintf(buffer,"h_%s_metget_et_%s_%s",tag_category[cate],sample[file].tag, type_category[type]);
	      h_metget_et[cate][file][type] = new TH2F(buffer,buffer, 120,0,3.0,60,0.,300.);
	      if(sumw2)h_metget_et[cate][file][type]->Sumw2();

	      sprintf(buffer,"h_%s_metdphi_et_%s_%s",tag_category[cate],sample[file].tag, type_category[type]);
	      h_metdphi_et[cate][file][type] = new TH2F(buffer,buffer, 100,0.0,TMath::Pi(),
						     60, 0.0, 300.);
...
// 	    if ( EvtInfo.sigmaIetaIeta[ipho]>0.002 ) continue;
	  ...
	  if ((cut_bits & (~(1<<_ECALISO)))==0){
	    ...
	    Float_t metphi = atan2(EvtInfo.metPy,EvtInfo.metPx);
	    Float_t phophi = EvtInfo.phi[ipho];
	    Float_t metphodphi = dPhi(metphi,phophi);

	    h_metdphi_et[cate][file][type]->Fill(metphodphi, EvtInfo.et[ipho],scaling_factor);
	    h_metget_et[cate][file][type]->Fill(EvtInfo.metEt/EvtInfo.et[ipho], EvtInfo.et[ipho],scaling_factor);
	  }
