...
//      hsig=(TH1F*)fZee->Get(hname);
    ...
    sprintf(hname,"h_%s_comb3Iso_sig_pt%d",EBEE, ptbin);
//     hsig = (TH1F*)ftemplate->Get(hname);
//     sprintf(hname,"h_%s_combIso",EBEE, ptbin);
//     hsig = (TH1F*)fZee->Get(hname);
  }
  ...
  if (strcmp(EBEE,"EB")==0 ) {
    if ( ptbin >= 50 ) {
      sprintf(hname,"h_%s_comb3Iso_bkg_pt%d",EBEE, 50);
//       hbkg = (TH1F*)ftemplate->Get(hname);  
    }else {
...
//       hbkg = (TH1F*)ftemplate->Get(hname);  
    ...
    if ( ptbin >= 60 ) {
      sprintf(hname,"h_%s_comb3IsoSB_EGdata_pt%d",EBEE, 60);
//       hbkg = (TH1F*)ftemplate->Get(hname);  
    }else {
      sprintf(hname,"h_%s_comb3IsoSB_EGdata_pt%d",EBEE, ptbin);
//       hbkg = (TH1F*)ftemplate->Get(hname);  
    }
  }    
...
//     	if ( ptbin==120 && i==(ni-1) ) { istep[2]=istep[3]=istep[4]=0.; sigma=5.;}
	...
	if ( para[10]-sigma10*tmp_errpara[10] < 1. ) 
      ...
      printf(" ************ \n");
      printf("  do %d th fit  \n", i);
//       if(i==4 && dataFile=="toy.dat")continue;
      arglist[0] = 500; // number of iteration
      gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);      
      gMinuit->mnmatu(1);
      if ( ierflg != 0 ) {
...
//     gMinuit->mnparm(3,  "exp tail"        , para[3] , TMath::Abs(para[3]*0.1) , para[3]-sigma*tmp_errpara[3]  , para[3]+sigma*tmp_errpara[3]  , ierflg);
//     gMinuit->mnparm(4,  "exg mean"        , para[4] , TMath::Abs(para[4]*0.1) , para[4]-sigma*tmp_errpara[4]  , para[4]+sigma*tmp_errpara[4]  , ierflg);
//     gMinuit->mnparm(5,  "exg width"       , para[5] , TMath::Abs(para[5]*0.1) , para[5]-sigma*tmp_errpara[5]  , para[5]+sigma*tmp_errpara[5]  , ierflg);
//     gMinuit->mnparm(6,  "constant"        , para[6] , TMath::Abs(para[6]*0.0) , para[6]-sigma*tmp_errpara[6]  , para[6]+sigma*tmp_errpara[6]  , ierflg);
//     gMinuit->mnparm(7,  "bg exp turnon"   , para[7] , TMath::Abs(para[7]*0.1) , para[7]-sigma*tmp_errpara[7]  , para[7]+sigma*tmp_errpara[7]  , ierflg);
//     gMinuit->mnparm(8,  "bg x offset  "   , para[8] , TMath::Abs(para[8]*0.1) , para[8]-sigma*tmp_errpara[8]  , para[8]+sigma*tmp_errpara[8]  , ierflg);
//     gMinuit->mnparm(9,  "bg bend slope"   , para[9] , TMath::Abs(para[9]*0.1) , para[9]-sigma*tmp_errpara[9]  , para[9]+sigma*tmp_errpara[9]  , ierflg);
//     gMinuit->mnparm(10, "bg bend power"   , para[10], TMath::Abs(para[10]*0.1), para[10]-sigma*tmp_errpara[10], para[10]+sigma*tmp_errpara[10], ierflg);
