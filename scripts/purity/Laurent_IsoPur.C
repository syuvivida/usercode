...
void Laurent_IsoPur::Loop(Int_t choiceEB) // 0 all  1 Barrel  2 Transition  3 Endcap
{
  ...
  Double_t cut = 1 ; // the cut we use to compute the three efficiencies  
  ...
  Double_t nAna[nSamples]; // number of analysed events - size of files
  ...
  TH1F* sumEffS;
  TH1F* sumEffB;
  TH1F* isoSw2[nPtBin]; // to feed to tfractionfitter to improve the fit
  TH1F* isoBw2[nPtBin];
  TH1* outMCpredS[nPtBin];
  TH1* outMCpredB[nPtBin];
  TArrayD* isoSw2A[nPtBin]; 
  TArrayD* isoBw2A[nPtBin];
  
  for(Int_t i=0; i<nPtBin; i++) {
    isoSw2A[i] = new TArrayD(nBins);
    isoBw2A[i] = new TArrayD(nBins);
  }
  ...
  Int_t entries[nSamples];
  Int_t fileStart[nSamples];
 ...
 Double_t GamEt[nPtBin+1]= { 15,20,27,35,45,57,72,90,120,150,200,300,400,550,1000}; 
 ...
 for(Int_t i=0; i< nPtBin; i++){ 
  ...
  for(Int_t samp=0; samp<nSamples; samp++) {
    ...
    if (TString(fileName[samp])!=""){
      ...
      sprintf(sochn2,"entries[%2i] %8i    %s",samp,entries[samp],fileName[samp]);
      cout << sochn2 << endl;
      fileStart[samp] = 0;
    }
  }
  ...
  cout << endl;
  for(Int_t samp=0; samp<nSamples; samp++) {
    sprintf(sochn2,"fileStart[%2i] %8i    %s",samp,fileStart[samp],fileName[samp]);
...
// 	if(thiset        >= GamEt[nPtBin])continue;
  ...
  // then print nAna
  for (Int_t i=0;i<nSamples;i++) {
    if(nAna[i]) cout << "nAna[" << i << "] : " << nAna[i] << endl;
...
//     Double_t scaleFactor = sc[i];
    Double_t scaleFactor = skimEff[i]*xSec[i]/nFill_ForIsoSum[i];
    cout << "scaleFactor for sample " << i <<  " = " << scaleFactor << endl;
    weightNormS  += skimEff[i]*xSec[i];
...
//     Double_t scaleFactor = sc[i];
  ...
  e[SDataIndex]->Scale(1000./weightNormS);
  h[SDataIndex]->Scale(1000./weightNormS);
  t[SDataIndex]->Scale(1000./weightNormS);
 
  e[BDataIndex]->Scale(1000./weightNormB);
  h[BDataIndex]->Scale(1000./weightNormB);
  t[BDataIndex]->Scale(1000./weightNormB);
  ...
  for (Int_t bin=1; bin<reachBins+1; bin++) {
    eEff->SetBinContent(bin,fabs(e[SDataIndex]->Integral(1,bin)/1000.-reachS));
    hEff->SetBinContent(bin,fabs(h[SDataIndex]->Integral(1,bin)/1000.-reachS));
    tEff->SetBinContent(bin,fabs(t[SDataIndex]->Integral(1,bin)/1000.-reachS));
  } 
  ...
  cout << "Efficiency for ecal background : " << 100*e[BDataIndex]->Integral(1,eEff->GetMinimumBin())/1000 << endl;
  ...
  cout << "Efficiency for hcal background : " << 100*h[BDataIndex]->Integral(1,hEff->GetMinimumBin())/1000 << endl;
  ...
  cout << "Efficiency for trck background : " << 100*t[BDataIndex]->Integral(1,tEff->GetMinimumBin())/1000 << endl;
  cout << endl;
...
//   	nFill[SDataIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(0) + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
  	nFill[SDataIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
...
//   	nFill[BDataIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(0) + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
  	nFill[BDataIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
...
//   	nFill[STemplateIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(0) + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
  	nFill[STemplateIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
      }
...
//  	nFill[BTemplateIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(0) + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
  	nFill[BTemplateIndex][ptb] += iso[i][ptb]->Integral() + iso[i][ptb]->GetBinContent(nBins+1) + antiSpike[i][ptb];
      }
  ...
  for (Int_t ptb=1; ptb<nPtBin; ptb++) {
  ...
  if (0) { 
    TCanvas* canPtHatDis = new TCanvas("PtHatDis","PtHatDis",800,800);
    canPtHatDis->SetLogy();
    ptHatDis[0]->SetMinimum(1e-10*ptHatDis[0]->GetMaximum());
    ptHatDis[0]->DrawCopy();
    ptHatDis[1]->SetLineColor(2);
    ptHatDis[1]->DrawCopy("same");
    ptHatDis[2]->DrawCopy("same");
    ptHatDis[3]->SetLineColor(2);
    ptHatDis[3]->DrawCopy("same");
    ptHatDis[4]->DrawCopy("same");
    ptHatDis[5]->SetLineColor(2);
    ptHatDis[5]->DrawCopy("same");
    ptHatDis[6]->DrawCopy("same");
  ...
  if(0) { // Drawing the 3 S&B distributions
  ...
  if(0) { // Drawing the 3 signal efficiencies
  ...
  if(0) { 
    ...
    canIso->cd(1)->SetLogy();
    Double_t intSDataIndex = iso[SDataIndex][0]->Integral()+iso[SDataIndex][0]->GetBinContent(nBins+1);
    Double_t intBDataIndex = iso[BDataIndex][0]->Integral()+iso[BDataIndex][0]->GetBinContent(nBins+1);
    
    iso[SDataIndex][0]->Scale(1./intSDataIndex);
    iso[BDataIndex][0]->Scale(1./intBDataIndex);
    
    for (Int_t i=1; i<nBins+1; i++) {
      sumEffS->SetBinContent(i,iso[SDataIndex][0]->GetMaximum()*iso[SDataIndex][0]->Integral(1,i));
      sumEffB->SetBinContent(i,iso[SDataIndex][0]->GetMaximum()*iso[BDataIndex][0]->Integral(1,i));
    }
    
    iso[SDataIndex][0]->GetXaxis()->SetTitle("SUM ISO");
    iso[SDataIndex][0]->SetLineColor(2);
    iso[BDataIndex][0]->SetLineColor(4);
    sumEffS->SetLineColor(2);
    sumEffS->SetLineStyle(2);
    sumEffB->SetLineColor(4);
    sumEffB->SetLineStyle(2);
    
    iso[SDataIndex][0]->DrawCopy("hist");
    iso[BDataIndex][0]->DrawCopy("hist same");
    ...
    iso[SDataIndex][0]->Scale(intSDataIndex);
    iso[BDataIndex][0]->Scale(intBDataIndex);
  }
