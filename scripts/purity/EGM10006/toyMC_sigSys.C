...
void toyMC_sigSys(int shift, int runIeta=-1, int runIpt=-1, int startExp=0){
...
//       fsum->Draw("same");
      ...
      ofstream dumpout;
      dumpout.open(Form("pull_bias%d_%d_%s_pt%d.dat",startExp, 
			shift,
	...
	htoyMC_data->Reset();
	ofstream fout;
	std::string toyData = Form("/tmp/syu/varysigtailtoy%d_%d_%d_%d.dat",
...
//   TFile* outFile = new TFile(Form("varySigTail%d_%d_%s_pt%d.root",
// 				  startExp,shift,
