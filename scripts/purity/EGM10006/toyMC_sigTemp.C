...
#include <TLegend.h>
#include <TROOT.h>
#include "Nov04_ext-mL_fit_ISO.C"
...
//   gSystem->mkdir("toysPlot");
  ...
  TFile* finFile = new TFile("/afs/cern.ch/user/s/syu/scratch0/bkgSys/template_comb3Iso_template.root");  
  TFile* zeeFile = new TFile("/afs/cern.ch/user/s/syu/scratch0/bkgSys/anadipho_Zee_Vg_3pb.root");
  ...
  for(int ieta=0; ieta< nEtaBin; ieta++){
    ...
    hTemplate_ShiftS[ieta]= (TH1D*)zeeFile->FindObjectAny(tmp);
	
    hTemplate_ShiftS[ieta]->Rebin(REBINNINGS_TEMP);
    ...
    for(int ipt=0; ipt < nPtBin; ipt++){
	 ...
	 finFile->GetName() << endl;
       ...
       FuncFitResult = Ifit("/afs/cern.ch/user/s/syu/scratch0/bkgSys/EGdata_comb3Iso_et.dat",
...
//        // loops over toys
	 ...
	 TH1D* htoyMC_sig  = (TH1D*)hTemplate_ShiftS[ieta]->Clone("htoyMC_sig");
// 	 htoyMC_sig->Reset();
...
// 	 htoyMC_bkg->Reset();
	 ...
	 htoyMC_data->Reset();
	 ofstream fout;
	 std::string toyData = Form("/tmp/syu/sigtemptoy_%d_%d.dat",ieta,ipt);
...
//  	}
  ...
  TFile* outFile = new TFile(Form("/afs/cern.ch/user/s/syu/scratch0/bkgSys/changeSigTemp_%s_pt%d.root",
