...
Double_t* Ifit(std::string dataFile, 
	       ...
	       int ptbin=30, char EBEE[10]="EB", int fit_data=2)
{
  ...
  if ( fit_data==1 ) {
	...
	if((strcmp(EBEE,"EB")==0 && TMath::Abs(xdata2)<1.5) ||
	   (strcmp(EBEE,"EE")==0 && TMath::Abs(xdata2)>1.5) ) {
...
//   fmcsigcorr->Draw("same");
  ...
  //f1->SetParameter(1,f1->GetParameter(1)*0.85);
//   f1->SetParameter(2,f1->GetParameter(2)+0.15);
//   f1->SetParameter(3,f1->GetParameter(3)+0.05);
  if ( strcmp(EBEE,"EB")==0 ){  
    f1->SetParameter(2,f1->GetParameter(2)-0.15);
    f1->SetParameter(3,f1->GetParameter(3)-0.15);
  }else{
    f1->SetParameter(2,f1->GetParameter(2)+0.08);
  ...
  if ( strcmp(EBEE,"EB")==0 ){  
//     f3->FixParameter(5,-0.45);
    f3->FixParameter(6,-0.060409);
...
//   if ( strcmp(EBEE,"EE")==0 ){  
//     if (f3->GetParameter(5)<-1.5) f3->SetParameter(5,-1.5);
//   }
...
//   gMinuit->mnparm(2,  "constant"     , Para[0], TMath::Abs(Para_err[0]*0.0),  Para[0], Para[0] , ierflg);
//   gMinuit->mnparm(3,  "exp tail"     , Para[1], TMath::Abs(Para_err[1]*0.0),  Para[1]-sigma*Para_err[1], Para[1]+sigma*Para_err[1], ierflg);
//   gMinuit->mnparm(4,  "exg mean"     , Para[2], TMath::Abs(Para_err[2]*0.1),  Para[2]-sigma*Para_err[2], Para[2]+sigma*Para_err[2], ierflg);
//   gMinuit->mnparm(5,  "exg width"    , Para[3], TMath::Abs(Para_err[3]*0.1),  Para[3]-sigma*Para_err[3], Para[3]+sigma*Para_err[3], ierflg);
//    gMinuit->mnparm(6,  "constant"     , Para[4], TMath::Abs(Para_err[4]*0.0),  Para[4]                  , Para[4]                  , ierflg);
//    gMinuit->mnparm(7,  "bg exp turnon", Para[5], TMath::Abs(Para_err[5]*0.1),  Para[5]-sigma*Para_err[5], Para[5]+sigma*Para_err[5], ierflg);
//    gMinuit->mnparm(8,  "bg x offset  ", Para[6], TMath::Abs(Para_err[6]*0.0),  Para[6]-sigma*Para_err[6], Para[6]+sigma*Para_err[6], ierflg);
//   gMinuit->mnparm(9,  "bg bend slope", Para[7], TMath::Abs(Para_err[7]*0.1),  Para[7]-sigma*Para_err[7], Para[7]+sigma*Para_err[7], ierflg);
//   gMinuit->mnparm(10, "bg bend power", Para[8], TMath::Abs(Para_err[8]*0.1),  Para[8]-sigma*Para_err[8], Para[8]+sigma*Para_err[8], ierflg);



  if ( strcmp(EBEE,"EB")==0 ) {
    sigma=.5; if(ptbin>=26) sigma=0.;
    gMinuit->mnparm(2,  "constant"     , Para[0], TMath::Abs(Para[0]*0.0) ,  Para[0], Para[0], ierflg);
//     gMinuit->mnparm(3,  "exp tail"     , Para[1], TMath::Abs(Para[1]*0.0) ,  0.9    , 1.2    , ierflg);
    gMinuit->mnparm(3,  "exp tail"     , Para[1], TMath::Abs(Para_err[1]*0.1*sigma),  Para[1]-sigma*Para_err[1], Para[1]+sigma*Para_err[1], ierflg);
    gMinuit->mnparm(4,  "exg mean"     , Para[2], TMath::Abs(Para[2]*0.1) ,  0.49   , 1.0    , ierflg);
...
//     gMinuit->mnparm(7,  "bg exp turnon", Para[5], TMath::Abs(Para[5]*0.0) ,  -0.7   , -0.3   , ierflg);
    gMinuit->mnparm(7,  "bg exp turnon", Para[5], TMath::Abs(Para_err[5]*0.1*sigma),  Para[5]-sigma*Para_err[5], Para[5]+sigma*Para_err[5], ierflg);
    gMinuit->mnparm(8,  "bg x offset  ", Para[6], TMath::Abs(Para[6]*0.0) ,  -0.15  , -0.0   , ierflg);
    gMinuit->mnparm(9,  "bg bend slope", Para[7], TMath::Abs(Para[7]*0.1) ,  0.01   , 0.05   , ierflg);
    
    sigma=1.;
    float min10=1.0; float max10=1.5; 
    if ( Para[8]-sigma*Para_err[8] > min10 ) min10 = Para[8]-sigma*Para_err[8];
    if ( Para[8]+sigma*Para_err[8] < max10 ) max10 = Para[8]+sigma*Para_err[8];
    if ( ptbin>=26 ) {
      gMinuit->mnparm(10, "bg bend power", Para[8], TMath::Abs(Para_err[8]*0.1),  min10,  max10, ierflg);
    }else {
      gMinuit->mnparm(10, "bg bend power", Para[8], TMath::Abs(Para[8]*0.1) ,  1.0    , 1.5    , ierflg);
    }

  }else{
    sigma=0.; Para[7]=0.02; if (ptbin==30 || ptbin==23 || ptbin==21) sigma=0.5;
    gMinuit->mnparm(2,  "constant"     , Para[0], TMath::Abs(Para[0]*0.0) ,  Para[0], Para[0], ierflg);
//     gMinuit->mnparm(3,  "exp tail"     , Para[1], TMath::Abs(Para[1]*0.0) ,  0.8    , 1.3    , ierflg);
    gMinuit->mnparm(3,  "exp tail"     , Para[1], TMath::Abs(Para_err[1]*0.1*sigma),  Para[1]-sigma*Para_err[1], Para[1]+sigma*Para_err[1], ierflg);
    gMinuit->mnparm(4,  "exg mean"     , Para[2], TMath::Abs(Para[2]*0.1) ,  0.0    , 0.1    , ierflg);
    gMinuit->mnparm(5,  "exg width"    , Para[3], TMath::Abs(Para[3]*0.1) ,  0.15   , 0.25    , ierflg);
    gMinuit->mnparm(6,  "constant"     , Para[4], TMath::Abs(Para[4]*0.0) ,  Para[4], Para[4], ierflg);
//     gMinuit->mnparm(7,  "bg exp turnon", Para[5], TMath::Abs(Para[5]*0.1) ,  -1.5   , -0.2   , ierflg);
    sigma=2.; if(ptbin==26 || ptbin==21) sigma=1.; if ( ptbin==23 ) sigma=0.;
    gMinuit->mnparm(7,  "bg exp turnon", Para[5], TMath::Abs(Para_err[5]*0.1*sigma),  Para[5]-sigma*Para_err[5], Para[5]+sigma*Para_err[5], ierflg);
    gMinuit->mnparm(8,  "bg x offset  ", Para[6], TMath::Abs(Para[6]*0.0) ,  -0.15  , -0.0   , ierflg);
    gMinuit->mnparm(9,  "bg bend slope", Para[7], TMath::Abs(Para[7]*0.1) ,  0.01   , 0.03   , ierflg);    

    sigma=2.; if(ptbin==26|| ptbin==21) sigma=1.; if ( ptbin==23 ) sigma=3.;
    float min10=1.0; float max10=5.; 
    if ( Para[8]-sigma*Para_err[8] > min10 ) min10 = Para[8]-sigma*Para_err[8];
    if ( Para[8]+sigma*Para_err[8] < max10 ) max10 = Para[8]+sigma*Para_err[8];
    if ( ptbin>=26 ) {
      gMinuit->mnparm(10, "bg bend power", Para[8], TMath::Abs(Para_err[8]*0.1),  min10,  max10, ierflg);
    }else {
      gMinuit->mnparm(10, "bg bend power", Para[8], TMath::Abs(Para[8]*0.1) ,  1.0    , 5.    , ierflg);
    }
//     sigma=0.; Para[7]=0.02; if (ptbin<=30) sigma=0.5; if (ptbin==26) sigma=0.;
//     sigma=2.; if(ptbin==26) sigma=1.; if ( ptbin==23 ) sigma=2.;
//     sigma=2.; if(ptbin==26) sigma=1.; if ( ptbin==23 ) sigma=2.;
  }
...
//     printf(" MIGRAD fit with yield only \n");

//     for(int j=0; j<=NPAR-1;j++) {
//       tmp_errpara[j] = errpara[j];
//       if( tmp_errpara[j] == 0. ) tmp_errpara[j] = TMath::Abs(par[j])*.2;      
//     }

//     sigma = 10.;
//     gMinuit->mnparm(0,  "Signal yield"    , para[0] , TMath::Abs(para[0]*0.1) , para[0]-sigma*tmp_errpara[0]  , para[0]+sigma*tmp_errpara[0]  , ierflg);
//     gMinuit->mnparm(1,  "background yield", para[1] , TMath::Abs(para[1]*0.1) , para[1]-sigma*tmp_errpara[1]  , para[1]+sigma*tmp_errpara[1]  , ierflg);
//     gMinuit->mnparm(2,  "constant"        , para[2] , TMath::Abs(para[2]*0.0) , para[2]-sigma*tmp_errpara[2]  , para[2]+sigma*tmp_errpara[2]  , ierflg);
//     gMinuit->mnparm(3,  "exp tail"        , para[3] , TMath::Abs(para[3]*0.0) , para[3]-sigma*tmp_errpara[3]  , para[3]+sigma*tmp_errpara[3]  , ierflg);
//     gMinuit->mnparm(4,  "exg mean"        , para[4] , TMath::Abs(para[4]*0.0) , para[4]-sigma*tmp_errpara[4]  , para[4]+sigma*tmp_errpara[4]  , ierflg);
//     gMinuit->mnparm(5,  "exg width"       , para[5] , TMath::Abs(para[5]*0.0) , para[5]-sigma*tmp_errpara[5]  , para[5]+sigma*tmp_errpara[5]  , ierflg);
//     gMinuit->mnparm(6,  "constant"        , para[6] , TMath::Abs(para[6]*0.0) , para[6]-sigma*tmp_errpara[6]  , para[6]+sigma*tmp_errpara[6]  , ierflg);
//     gMinuit->mnparm(7,  "bg exp turnon"   , para[7] , TMath::Abs(para[7]*0.0) , para[7]-sigma*tmp_errpara[7]  , para[7]+sigma*tmp_errpara[7]  , ierflg);
//     gMinuit->mnparm(8,  "bg x offset  "   , para[8] , TMath::Abs(para[8]*0.0) , para[8]-sigma*tmp_errpara[8]  , para[8]+sigma*tmp_errpara[8]  , ierflg);
//     gMinuit->mnparm(9,  "bg bend slope"   , para[9] , TMath::Abs(para[9]*0.0) , para[9]-sigma*tmp_errpara[9]  , para[9]+sigma*tmp_errpara[9]  , ierflg);
//     gMinuit->mnparm(10, "bg bend power"   , para[10], TMath::Abs(para[10]*0.0), para[10]-sigma*tmp_errpara[10], para[10]+sigma*tmp_errpara[10], ierflg);
 
//     gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);      
//     printf("         --------------------------------------------------------- \n");
//     printf("         Done MIGRAD.  ptbin %d ierr = %d \n", ptbin, ierflg);
...
//   }
  ...
  if ( ierflg == 0 ) {
    ...
    for(int j=0; j<=NPAR-1;j++) {
      tmp_errpara[j] = errpara[j];
      if( tmp_errpara[j] == 0. ) tmp_errpara[j] = TMath::Abs(par[j])*.1;      
    }
...
//  //   gMinuit->mnparm(3,  "exp tail"     , Para[1],  0.01,  0.7    , 1.5    , ierflg);
//  //   gMinuit->mnparm(4,  "exg mean"     , Para[2],  0.01,  0.3    , 1.1    , ierflg);
//  //   gMinuit->mnparm(5,  "exg width"    , Para[3],  0.01,  0.1    , 0.6    , ierflg);
//  //   gMinuit->mnparm(6,  "constant"     , Para[4],  0.00,  Para[4], Para[4], ierflg);
//  //   gMinuit->mnparm(7,  "bg exp turnon", Para[5],  0.01,  -0.8   , -0.2   , ierflg);
//  //   gMinuit->mnparm(8,  "bg x offset  ", Para[6],  0.01,  -0.2   , -0.01  , ierflg);
//  //   gMinuit->mnparm(9,  "bg bend slope", Para[7],  0.01,  0.001  , 0.1    , ierflg);
//  //   gMinuit->mnparm(10, "bg bend power", Para[8],  0.01,  0.5    , 2.     , ierflg);
//     //if ( ptbin < 60 ) {
//       gMinuit->mnexcm("MINOS", arglist ,3,ierflg);
//       printf("         --------------------------------------------------------- \n");
//       printf("         Done MINOS.  ptbin %d ierr = %d \n", ptbin, ierflg);
...
//       cout << f11->GetParameter(epar) << "," << endl;
...
//       cout << f12->GetParameter(epar) << "," << endl;
...
//       cout << f13->GetParameter(epar) << "," << endl;
