...
void readPullBias2(char*prefix="pull_bias"){
  ...
  for(int ieta=0; ieta < 2; ieta++){
	...
	for(int irun=0; irun < NRUNS; irun++){
	  sprintf(tmp,"%s%d_%d_%s_pt%d.dat",
		  prefix,irun,ishift,
		  dec[ieta],fBinsPt[ipt]);		
	...
	hpull->SetTitle(Form("%s, pt = %d-%d GeV, %d #sigma",dec[ieta],
			     fBinsPt[ipt],fBinsPt[ipt+1], ishift));      
	hbias->SetTitle(Form("%s, pt = %d-%d GeV, %d #sigma",dec[ieta],
			     fBinsPt[ipt],fBinsPt[ipt+1], ishift));

	if(hpull->GetEntries()<1)continue;
	hpull->Fit("gaus");
	TF1* f1 = hpull->GetFunction("gaus");
	foutm << dec[ieta] << " pt" << fBinsPt[ipt] << " " << ishift 
	      << " sigma, pull: " 
	      << f1->GetParameter(1) << " +- " << f1->GetParError(1) << endl;
	fouts << dec[ieta] << " pt" << fBinsPt[ipt] << " " << ishift 
	      << " sigma, pull: " 
	      << f1->GetParameter(2) << " +- " << f1->GetParError(2) << endl;
	c1->Print(Form("figures_%s/pull_%s_%d_%s_pt%d.png",
		       prefix,prefix, ishift,
		       dec[ieta],fBinsPt[ipt]));
	...
	foutm << dec[ieta] << " pt" << fBinsPt[ipt] << " " << ishift 
	      << " sigma, bias: " 
	      << f2->GetParameter(1) << " +- " << f2->GetParError(1) << endl;
	fouts << dec[ieta] << " pt" << fBinsPt[ipt] << " " << ishift 
	      << " sigma, bias: " 
	      << f2->GetParameter(2) << " +- " << f2->GetParError(2) << endl;
	c1->Print(Form("figures_%s/bias_%s_%d_%s_pt%d.png",
		       prefix,prefix, ishift, 
		       dec[ieta],fBinsPt[ipt]));

      } // end of loop over shift
