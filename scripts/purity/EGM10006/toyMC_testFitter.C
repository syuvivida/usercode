...
//   gSystem->mkdir("toysPlot");
  ...
  TFile* finFile = new TFile("/afs/cern.ch/user/s/syu/scratch0/testArea/template_comb3Iso_template.root");  
...
// 	  (ieta==1 && ipt<9))
	 hTemplate_B[ieta][ipt]->Rebin(REBINNINGS_TEMP);
       ...
       FuncFitResult = Ifit("/afs/cern.ch/user/s/syu/scratch0/testArea/EGdata_comb3Iso_et.dat",
...
// 	 htoyMC_bkg->Reset();
	 ...
	 htoyMC_data->Reset();
	 ofstream fout;
	 std::string toyData = Form("/afs/cern.ch/user/s/syu/scratch0/testArea/toy_%d_%d.dat",ieta,ipt);
