...
void toyMC_varyTurnOn(int shift, int runIeta=-1, int runIpt=-1, int startExp=0)
{
  ...
  for(int ieta=0; ieta< nEtaBin; ieta++){
	...
	sumFitPar[ipar] = myFitPar[ipar+NPAR*2];
      ...
      current = fbkg->GetParameter(5) + (Double_t)shift*fShiftTurnOn[ieta][ipt];
      fbkg->SetParameter(5, current);
...
//       fsum->Draw("same");
      ...
      ofstream dumpout;
      dumpout.open(Form("varyturnon_pull_bias%d_%d_%s_pt%d.dat",
			startExp, shift,
	...
	htoyMC_data->Reset();
	ofstream fout;
	std::string toyData = Form("/tmp/syu/varyturnontoy%d_%d_%d_%d.dat",
...
//   TFile* outFile = new TFile(Form("varyTurnOn%d_%d_%s_pt%d.root",
// 				  startExp, shift,
