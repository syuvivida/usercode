...
#include <TLegend.h>
#include <TROOT.h>
#include "VaryTurnOn_ext-mL_fit_ISO.C"
...
void toyMC_varyTurnOn(int shift, int runIeta, int runIpt){
...
//   gSystem->mkdir("toysPlot");
  ...
  TFile* finFile = new TFile("/afs/cern.ch/user/s/syu/scratch0/VaryShape/template_comb3Iso_template.root");  
...
// 	  (ieta==1 && ipt<9))
	 hTemplate_B[ieta][ipt]->Rebin(REBINNINGS_TEMP);
       ...
       Double_t dataValue, dataValueErr;
       FuncFitResult = Ifit(0, dataValue, dataValueErr,
			    "/afs/cern.ch/user/s/syu/scratch0/VaryShape/EGdata_comb3Iso_et.dat",
...
// 	 htoyMC_bkg->Reset();
	 ...
	 htoyMC_data->Reset();
	 ofstream fout;
	 std::string toyData = Form("/tmp/syu/varyturntoy%d_%d_%d.dat",shift,ieta,ipt);
...
// 	   nbkggen << endl;
	 ...
	 toyFitResult =  Ifit(shift, dataValue, dataValueErr,
			      toyData.data(), 
...
// 	}
  ...
  TFile* outFile = new TFile(Form("varyTurnOn%d_%s_pt%d.root",
				  shift,
