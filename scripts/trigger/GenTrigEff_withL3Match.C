...
void GenTrigEff_withL3Match(TChain* pho, 
	  std::string filestring, bool isBarrel)
{
//   int nbin=200;
  int nbin=100;
  float xmin=0.0;
  ...
  for( Long64_t j=0; j< nentries; j++){
    ...
    for(int i=0; i<GenInfo.Size; i++)
      {
 	float thisEt = GenInfo.Pt[i];
	float thisEta = GenInfo.Eta[i];
  	int HLT = GenInfo.Trig[i];
	...
	if(isBarrel && fabs(thisEta)>1.44)continue;
	ncount[1]++;
	if(!isBarrel && (fabs(thisEta) < 1.56 || fabs(thisEta) > 2.50))
	  continue;	
	ncount[2]++;
	
	bool hasLoosePhoton = true;

	// matched to a genp photon
	int pid = GenInfo.PID[i];
	int mpid = GenInfo.MPID[i]; 
...
// 	// Photon_20_Iso
	...
	// L1EG5 relative to reconstructed photon
  	if(hasLoosePhoton)
		...
		h_gengetdeno->Fill(GenInfo.Pt[i]);
	      }

  	    if(isFromHard && passL1EG5Trigger)
	      {
		h_get5numr->Fill(thisEt);
		h_gengetnumr->Fill(GenInfo.Pt[i]);
	      }
  ...
  std::string histoFile =  "/home/syu/Photon_Code/CMSSW_3_1_2/src/syu/MyObjectCounter/test/script_output/test_" + filestring + "_" + detector + "_default.root";
