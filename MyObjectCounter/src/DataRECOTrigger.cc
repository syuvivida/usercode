...
#include "syu/MyObjectCounter/header/DataRECOTrigger.hh" 
#include <algorithm>


DataRECOTrigger::DataRECOTrigger(const edm::ParameterSet& iConfig):
...
DataRECOTrigger::~DataRECOTrigger()
{
}

void DataRECOTrigger::beginJob()
{
  edm::Service<TFileService> fs;
  TFileDirectory results = TFileDirectory( fs->mkdir("DataRECOTrigger") );
...
void DataRECOTrigger::endJob() 
{
  std::cout << "DataRECOTrigger has " << _nIn << " input events and " << _nOut  << " output events" << endl;
}


// analyzing reconstructed electrons, muons, and photons

void DataRECOTrigger::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
DEFINE_FWK_MODULE(DataRECOTrigger);
